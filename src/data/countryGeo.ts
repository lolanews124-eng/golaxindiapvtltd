...
/** hreflang map across all country landing pages */
export function buildInternationalHreflang(
  pathForCountry: (countrySlug: string) => string,
  xDefaultPath?: string,
): Record<string, string> {
  const languages: Record<string, string> = {};
  for (const [slug, geo] of Object.entries(countryGeoMap)) {
    languages[geo.hreflang] = pathForCountry(slug);
  }
  languages["x-default"] = xDefaultPath ?? pathForCountry("united-states");
  return languages;
}
