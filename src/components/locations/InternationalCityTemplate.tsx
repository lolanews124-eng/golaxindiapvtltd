...
const services = [
  ...
  { icon: TrendingUp, title: "Digital Marketing & SEO", slug: "digital-marketing", desc: "Performance marketing, SEO & content engines" },
...
export default function InternationalCityTemplate({ data }: Props) {
  ...
  const unique = getCityPageContent(country.slug, citySlug);
  ...
  const introHeading = unique?.introHeading ?? `Trusted IT & Digital Partner for ${city} Businesses`;
  const introParagraphs = unique?.intro ?? [
    ...
    <Layout>
      ...
      <JsonLd data={{
            ...
            "@graph": [
              {
                "@type": "Service",
                "@id": `${SITE}${pagePath}#service`,
                name: `Offshore IT & Web Development for ${city}, ${country.country}`,
                url: `${SITE}${pagePath}`,
                description,
                serviceType: [
                  "Web Development",
      ...
      <section className="py-16 bg-background">
        <div className="container mx-auto px-4 max-w-4xl">
          <h2 className="text-3xl md:text-4xl font-bold mb-6">{introHeading}</h2>
          <div className="prose prose-lg max-w-none text-muted-foreground space-y-4">
            {introParagraphs.map((para) => (
              <p key={para.slice(0, 48)}>{para}</p>
            ))}
          </div>
          {unique?.localFocus && unique.localFocus.length > 0 && (
            <ul className="mt-8 grid sm:grid-cols-2 gap-3">
              {unique.localFocus.map((item) => (
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
            {services.map((s, i) => (
              ...
              >
                ...
                <Link href={getServiceCountryHref(s.slug, country.slug)} className="text-primary text-sm font-medium inline-flex items-center gap-1 hover:gap-2 transition-all">
      ...
      {/* FAQs — unique cities only (templated FAQs stay in JSON-LD) */}
      {unique && unique.faqs.length > 0 && (
        <section className="py-16 bg-background">
          <div className="container mx-auto px-4 max-w-4xl">
            <h2 className="text-3xl md:text-4xl font-bold mb-8">FAQs for {city} Teams</h2>
