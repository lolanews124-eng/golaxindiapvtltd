...
export default function ServiceInternationalTemplate({ data }: Props) {
  ...
  const unique = getServiceCountryContent(service.slug, location.slug);
  ...
  const longFormSections = unique
    ...
    : ([
      ...
      ] as const).map(([key, heading], i, arr) => ({
        heading,
        body: seoContent[key],
        emphasize: i === arr.length - 1,
      }));

  const siblingCountries = internationalLocations.filter(
    (l) => l.slug !== location.slug && isServiceCountryAllowed(service.slug, l.slug),
  );
      ...
      <section className="py-20 bg-gradient-subtle">
        <div className="container mx-auto px-4">
          <div className="max-w-4xl mx-auto">
            {longFormSections.map((section, i) => (
              <motion.div
                key={section.heading}
                initial={{ opacity: 0, y: 20 }}
                whileInView={{ opacity: 1, y: 0 }}
                viewport={{ once: true }}
                className={`mb-10 ${section.emphasize ? "bg-gradient-hero rounded-2xl p-8 text-primary-foreground" : ""}`}
              >
                <h3 className={`font-heading text-2xl font-bold mb-4 ${section.emphasize ? "text-primary-foreground" : "text-foreground"}`}>
                  {section.heading}
                </h3>
                <p className={`leading-relaxed whitespace-pre-line ${section.emphasize ? "text-primary-foreground/90" : "text-muted-foreground"}`}>
                  {section.body}
                </p>
              </motion.div>
            ))}
      ...
      <section className="py-20 bg-gradient-subtle">
        <div className="container mx-auto px-4">
          ...
          <div className="grid sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 max-w-6xl mx-auto">
            {siblingCountries.map((l) => (
                <Link
                  key={l.slug}
                  href={getServiceCountryHref(service.slug, l.slug)}
